...
function StudyTimetable({ user }: { user: User }) {
  ...
  const extendSession = (id: number, minutes: number, targetDeductId: number | 'none') => {
    ...
    // SILENT EXTENSION LOG FOR FIREBASE EMAIL
  ...
  return (
    <div className="tt-root">
      ...
      {/* NEW SLEEK, SOLID EXTENSION MODAL (SAFE CSS) */}
      {extendModal && (() => {
        ...
        return (
          <div className="tt-extOverlay" onClick={() => setExtendModal(null)}>
            <div className="tt-extBox" onClick={(e) => e.stopPropagation()}>
              <div className="tt-extHeader">
                <div className="tt-extIcon">{row?.icon || "⏱"}</div>
                <div>
                  <h4>Tactical Time Extension</h4>
                  <h2>{row?.act}</h2>
                </div>
              </div>
              
              <div className="tt-extBody">
                ...
                <div className="tt-extInputWrap">
                  <button onClick={() => setExtendMins(Math.max(1, extendMins - 5))}>-</button>
                  <input type="number" value={extendMins} onChange={(e) => setExtendMins(Math.max(1, Number(e.target.value)))} min="1" />
                  <button onClick={() => setExtendMins(extendMins + 5)}>+</button>
                </div>
              ...
              <div className="tt-extFooter">
                <button className="tt-extCancel" onClick={() => setExtendModal(null)}>Cancel</button>
                <button className="tt-extConfirm" onClick={() => { extendSession(extendModal.id, extendMins, deductId); setExtendModal(null); setDeductId('none'); }}>Confirm +{extendMins}m</button>
              </div>
            </div>
          </div>
        );
      ...
      <style>{`
        ...
        /* SLEEK, SAFE EXTENSION MODAL CSS */
        .tt-extOverlay { position: fixed; inset: 0; background: rgba(15, 23, 42, 0.8); z-index: 9999; display: flex; justify-content: center; align-items: center; padding: 20px; animation: fadeIn 0.2s; }
        .tt-extBox { background: #fff; width: 100%; max-width: 450px; border-radius: 16px; overflow: hidden; box-shadow: 0 20px 40px rgba(0,0,0,0.4); animation: slideUp 0.3s cubic-bezier(0.16, 1, 0.3, 1); }
        .tt-extHeader { background: #1f2870; color: #fff; padding: 20px 24px; display: flex; align-items: center; gap: 15px; }
        .tt-extIcon { font-size: 32px; background: rgba(255,255,255,0.1); width: 56px; height: 56px; display: flex; align-items: center; justify-content: center; border-radius: 12px; border: 1px solid rgba(255,255,255,0.2); }
        .tt-extHeader h4 { margin: 0; font-size: 12px; text-transform: uppercase; color: #93c5fd; letter-spacing: 1px; }
        .tt-extHeader h2 { margin: 4px 0 0 0; font-size: 20px; font-weight: 800; color: #fff; }
        
        .tt-extBody { padding: 24px; background: #f8fafc; }
        .tt-extBody label { display: block; font-size: 13px; font-weight: 700; color: #475569; margin-bottom: 10px; text-transform: uppercase; letter-spacing: 0.5px; }
        .tt-extBtnGroup { display: flex; gap: 8px; margin-bottom: 12px; }
        .tt-extBtnGroup button { flex: 1; padding: 10px 0; border: 2px solid #e2e8f0; background: #fff; color: #475569; font-weight: 700; border-radius: 8px; cursor: pointer; transition: 0.2s; }
        .tt-extBtnGroup button.active { border-color: #3b82f6; background: #eff6ff; color: #1d4ed8; }
        .tt-extInputWrap { display: flex; align-items: center; gap: 10px; background: #fff; border: 2px solid #e2e8f0; border-radius: 8px; padding: 4px; }
        .tt-extInputWrap button { width: 40px; height: 40px; background: #f1f5f9; border: none; border-radius: 6px; font-size: 20px; font-weight: bold; color: #334155; cursor: pointer; }
        .tt-extInputWrap button:hover { background: #e2e8f0; }
        .tt-extInputWrap input { flex: 1; text-align: center; border: none; font-size: 24px; font-weight: 800; color: #1f2870; outline: none; }
        
        .tt-extSelect { width: 100%; padding: 14px; border: 2px solid #e2e8f0; border-radius: 8px; font-size: 14px; font-weight: 600; color: #334155; outline: none; background: #fff; cursor: pointer; }
        .tt-extSelect:focus { border-color: #3b82f6; }
        
        .tt-extFooter { padding: 20px 24px; background: #fff; border-top: 1px solid #e2e8f0; display: flex; gap: 12px; justify-content: flex-end; }
        .tt-extCancel { padding: 12px 20px; border: none; background: #f1f5f9; color: #475569; font-weight: 700; border-radius: 8px; cursor: pointer; font-size: 14px; transition: 0.2s; }
        .tt-extCancel:hover { background: #e2e8f0; }
        .tt-extConfirm { padding: 12px 24px; border: none; background: #22c55e; color: #fff; font-weight: 700; border-radius: 8px; cursor: pointer; font-size: 14px; transition: 0.2s; box-shadow: 0 4px 12px rgba(34, 197, 94, 0.3); }
        .tt-extConfirm:hover { background: #16a34a; transform: translateY(-1px); }
