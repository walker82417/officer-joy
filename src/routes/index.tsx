...
function StudyTimetable({ user }: { user: User }) {
  ...
  // LIVE PROGRESS MATH (Safe Logic Injection)
  ...
  const completeSession = (id: number) => {
    ...
    // FIREBASE SYNC BUG FIX (Forces save so email engine reads accurate data)
  ...
  const extendSession = (id: number, minutes: number, targetDeductId: number | 'none') => {
    ...
    // PENDING MISSIONS MATH FIX (Reduces durationAllocated so Pending Missions banner updates)
  ...
  return (
    <div className="tt-root">
                ...
                <div className="tt-pendingList">
                  {pending.length === 0 ? (
                    ...
                    pending.map((id) => {
                      ...
                      if (!r) return null;
                      // Reads the newly updated durationAllocated (mathematically reduced)
      ...
      {/* EXTENSION MODAL — Safe Scoped Glass Morphism */}
      {extendModal && (() => {
      ...
      {/* PERFECT UNMODIFIED ORIGINAL CSS WITH NEW MODAL APPENDED */}
      <style>{`
        ...
        .tt-glassStepper button:hover { background: rgba(31,40,112,0.08); }
