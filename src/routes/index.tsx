...
function StudyTimetable({ user }: { user: User }) {
  ...
  return (
    <div className="tt-root">
      ...
      {/* PERFECT EDGE-TO-EDGE CSS */}
      <style>{`
        body, html { margin: 0; padding: 0; background: #fff; }
        .tt-root { font-family: 'Inter', sans-serif; width: 100%; min-height: 100vh; }
        .tt-scaleWrap { width: 100%; }
        .tt-app { width: 100%; }
        
        .tt-examStrip { display: flex; width: 100%; }
        .tt-examBox { flex: 1; padding: 12px; text-align: center; color: #fff; cursor: pointer; position: relative; }
        .tt-examBox.ssc { background: #059669; }
        .tt-examBox.gate { background: #2563eb; }
        .tt-examBox.ese { background: #dc2626; }
        .tt-examBox .tt-num { font-size: 22px; font-weight: 800; font-family: 'Oswald', sans-serif; letter-spacing: 1px; }
        .tt-examBox .tt-lbl { font-size: 13px; font-weight: 700; margin-top: 2px; }
        .tt-examBox .tt-sub { font-size: 11px; opacity: 0.8; }
        ...
        .tt-examEdit input { flex: 1; padding: 4px; font-size: 12px; color: #111; }
        ...
        .tt-header { width: 100%; margin-bottom: 20px; }
        .tt-headerTop { display: flex; justify-content: space-between; align-items: center; padding: 15px 30px; background: #fff; color: #111; border-bottom: 2px solid #1f2870; }
        
        .tt-rulesList { text-align: left; font-size: 12px; font-weight: 700; color: #334155; line-height: 1.6; }
        .tt-rulesList span { color: #22c55e; margin-right: 6px; }
        
        .tt-titleBlock { text-align: center; flex: 2; }
        .tt-titleBlock h1 { margin: 0 0 8px 0; font-size: 32px; font-family: 'Oswald', sans-serif; color: #1f2870; letter-spacing: 1px; }
        .tt-examTags { font-size: 13px; font-weight: 800; display: inline-block; padding: 6px 15px; border-radius: 4px; background: #1f2870; color: #fff; margin-bottom: 10px; }
        .tt-examTags b { color: #fff; margin: 0 4px; } 
        
        .tt-motto { background: #1f2870; color: #f0b429; font-weight: bold; font-size: 14px; padding: 8px 0; letter-spacing: 2px; width: 100%; text-align: center; }

        .tt-targetBlock { text-align: right; font-size: 11px; font-weight: 800; line-height: 1.6; flex: 1; }
        .tt-targetBlock .t1 { color: #dc2626; } .tt-targetBlock .t2 { color: #2563eb; } .tt-targetBlock .t3 { color: #059669; }

        .tt-liveRow { display: flex; justify-content: space-between; align-items: center; background: #1f2870; color: #fff; padding: 8px 30px; border-top: 1px solid rgba(255,255,255,0.1); }
        .tt-greet { font-size: 14px; font-weight: 700; }
        .tt-clock { font-size: 14px; font-weight: 700; font-family: monospace; color: #f0b429; }

        .tt-quoteBar { text-align: center; padding: 12px; font-size: 13px; font-style: italic; color: #475569; font-weight: 500; }
        
        .tt-syncIndicatorWrap { display: flex; justify-content: flex-end; padding: 0 30px 10px; }
        .tt-syncIndicator { background: #f0fdf4; color: #166534; border: 1px solid #bbf7d0; padding: 6px 15px; border-radius: 20px; display: inline-flex; align-items: center; gap: 8px; font-size: 12px; font-weight: bold; }
        .tt-syncDot { background: #22c55e; width: 8px; height: 8px; border-radius: 50%; }

        .tt-mainGrid { padding: 0 30px; }
        .tt-table { width: 100%; border-collapse: collapse; font-size: 13px; margin-bottom: 20px; }
        .tt-table th { background: #1f2870; color: #fff; padding: 12px 10px; font-size: 11px; text-transform: uppercase; letter-spacing: 1px; }
        .tt-table td { padding: 12px 10px; border-bottom: 1px solid #e2e8f0; vertical-align: middle; color: #334155; }
        ...
        .tt-rowDONE td { background: #f0fdf4; opacity: 0.7; text-decoration: line-through; }
        .tt-rowDONE td.tt-actBtns { text-decoration: none; }
        .tt-rowIcon { font-size: 16px; text-align: center; width: 40px; }
        
        .tt-statusPill { font-size: 10px; font-weight: bold; padding: 4px 8px; border-radius: 4px; border: 1px solid currentColor; }
        ...
        .tt-rowTimer { font-family: monospace; font-size: 15px; font-weight: bold; color: #1e293b; }
        
        .tt-actBtns { display: flex; gap: 6px; }
        .tt-actBtns button { width: 28px; height: 28px; border: none; border-radius: 4px; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 12px; color: #fff; transition: 0.2s; }
        ...
        .tt-pendingBox { background: #fffbeb; border: 1px solid #fde68a; border-radius: 6px; padding: 12px 20px; margin-bottom: 20px; display: flex; align-items: center; gap: 15px; }
        .tt-pendingBox h3 { margin: 0; font-size: 13px; color: #d97706; text-transform: uppercase; white-space: nowrap; }
        .tt-pendingList { display: flex; flex-wrap: wrap; gap: 10px; flex: 1; }
        .tt-pendingItem { background: #fff; padding: 6px 12px; border-radius: 4px; font-size: 12px; font-weight: 600; border: 1px solid #fcd34d; display: flex; align-items: center; gap: 8px; color: #334155; }
        .tt-pendingItem button { background: #ea580c; color: white; border: none; padding: 4px 8px; border-radius: 4px; cursor: pointer; font-size: 10px; font-weight: bold; }
        
        .tt-bottomGrid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; align-items: start; }
        .tt-card { border: 1px solid #e2e8f0; border-radius: 6px; overflow: hidden; background: #fff; margin-bottom: 20px; }
        .tt-card h3 { background: #f8fafc; margin: 0; padding: 10px 15px; font-size: 12px; color: #475569; border-bottom: 1px solid #e2e8f0; text-transform: uppercase; letter-spacing: 0.5px; }
        .tt-cardBody { padding: 15px; font-size: 12px; color: #334155; }
        
        .tt-rotationTable { width: 100%; border-collapse: collapse; }
        .tt-rotationTable td { padding: 8px; border-bottom: 1px solid #f1f5f9; }
        ...
        .tt-analyticsGrid > div { background: #f8fafc; padding: 15px 10px; border-radius: 6px; text-align: center; font-size: 10px; color: #64748b; font-weight: bold; border: 1px solid #e2e8f0; }
        .tt-analyticsGrid b { display: block; font-size: 18px; color: #1f2870; margin-bottom: 4px; font-family: Oswald, sans-serif; }
        
        .tt-examCoverage { list-style: none; padding: 0; margin: 0; display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }
        .tt-examCoverage li { color: #334155; font-weight: 600; font-size: 11px; }
        .tt-examCoverage li:before { content: "✓"; color: #22c55e; font-weight: bold; display: inline-block; width: 1.2em; }
        
        .tt-ringWrap { display: flex; align-items: center; justify-content: center; padding: 20px; gap: 20px; }
        .tt-ringCanvas { width: 90px; height: 90px; }
        .tt-statList { font-size: 12px; color: #475569; line-height: 1.6; }
        .tt-statList b { color: #1f2870; font-size: 14px; }
        
        .tt-goldenRules { list-style: none; padding: 0; margin: 0; display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }
        .tt-goldenRules li { font-weight: 600; color: #1f2870; font-size: 11px; }
        ...
        .tt-emailCard p { padding: 15px; font-size: 11px; color: #64748b; margin: 0; line-height: 1.5; }
        
        .tt-heatmapWrap { padding: 15px; }
        .tt-heatmapGrid { display: grid; grid-template-columns: repeat(12, 1fr); gap: 3px; margin-bottom: 10px; }
        ...
        .tt-checklist label { display: flex; align-items: center; gap: 8px; font-size: 12px; color: #334155; cursor: pointer; font-weight: 500; }
        .tt-checklist input { width: 14px; height: 14px; cursor: pointer; }
        ...
        .tt-tmBtns button { flex: 1; padding: 12px 0; border: none; border-radius: 8px; font-weight: bold; font-size: 14px; cursor: pointer; transition: 0.2s; color: white; }
        .tt-tmBtns button:disabled { opacity: 0.5; cursor: not-allowed; }
        .tt-tmCloseBtn { background: #e2e8f0; color: #475569; border: none; padding: 6px 12px; border-radius: 8px; font-size: 12px; font-weight: bold; cursor: pointer; transition: 0.2s; }
        .tt-tmCloseBtn:hover { background: #cbd5e1; }
      `}</style>
    </div>
  );
}
